...
### טבלאות ה-IMDb בשימוש

IMDb מפרסם מספר טבלאות נתונים ציבוריות, ומתוכן נעשה שימוש בארבע:

1. **טבלת פרטי הכותרות** — מכילה לכל סרט/סדרה את סוג הכותרת, השם, שנת היציאה, משך הזמן ורשימת הז'אנרים. זו טבלת הבסיס שממנה מסוננים הסרטים הרלוונטיים: רק סרטי קולנוע (לא סדרות), בטווח השנים והאורך הרצויים, ובטווח האותיות הראשונות שהוקצה לקבוצה.
2. **טבלת הדירוגים** — מכילה לכל כותרת את ממוצע הדירוג (averageRating) ואת מספר המצביעים (numVotes). הטבלה מצורפת לרשימת הסרטים שנבחרה, כך שמתווספות עמודות הדירוג לכל סרט — תוך שמירה על סרטים שעדיין אין להם דירוג.
3. **טבלת הצוות (קרדיטים)** — מכילה לכל סרט את כל אנשי הצוות המופיעים בו (שחקנים, במאים וכו') לפי סדר קרדיט. ממנה מחולצים עד 5 השחקנים המובילים בכל סרט, ונשמרים מזהיהם.
4. **טבלת פרטי האנשים** — מכילה מידע על אנשי קולנוע, ובכללם שנת לידה. טבלה זו מורדת כבר בשלב הזה לטובת חלק 2 של הפרויקט, שם היא משמשת לחישוב קרבת השחקנים לגיל שיא הקריירה שלהם.

מעבר לנתוני IMDb, נאספים גם נתוני Wikipedia (שפה, מדינה, תקציב, הכנסות ועלילה) באמצעות web scraping, כמתואר בהמשך.
